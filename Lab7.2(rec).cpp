...
int arrSearchRow(int** P, const int rowCount, const int colCount, int maxEl, int i) {
    maxEl = arrSearchCol(P, colCount, rowCount, maxEl, 0);
    if (i < rowCount - 1) {
        arrSearchRow(P, rowCount, colCount, maxEl, i + 1);
    }
    else
        return maxEl;
}
...
int main()
{
    ...
    int maxEl = colMax(P, rowCount, 0, P[0][0], 0);
    
    cout << arrSearchRow(P, rowCount, colCount, maxEl, 0) << endl;
